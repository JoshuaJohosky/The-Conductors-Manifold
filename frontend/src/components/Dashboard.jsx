...
/**
 * Multi-scale comparison view
 */
const MultiscaleView = ({ data }) => {
  const [selectedScale, setSelectedScale] = useState('daily');

  if (!data || !data.scales) return null;

  const scales = Object.keys(data.scales);
  const currentData = data.scales[selectedScale];

  return (
    <div className="multiscale-view">
      <h2>Multi-Timeframe Analysis: {data.symbol}</h2>

      <div className="scale-tabs">
        {scales.map(scale => (
          <button
            key={scale}
            className={`scale-tab ${scale === selectedScale ? 'active' : ''}`}
        ...
        <div className="scale-content">
          <div className="scale-metrics">
            <div className="metric-card">
              ...
              <div className="metric-value">{currentData.entropy.toFixed(2)}</div>
            </div>
            <div className="metric-card">
              <h3>Singularities</h3>
              <div className="metric-value">{currentData.singularities.length}</div>
            </div>
            <div className="metric-card">
              <h3>Attractors</h3>
              <div className="metric-value">{currentData.attractors.length}</div>
            </div>
          </div>

          <ManifoldViewer3D manifoldData={currentData} width={900} height={500} />
        </div>
      )}

      <div className="fractal-insight">
        <h3>Fractal Consistency</h3>
        <p>Observe how patterns echo across scales - the monthly fold contains the weekly flow, which contains the daily pulse.</p>
      </div>
    </div>
  );
};
